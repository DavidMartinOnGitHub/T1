...
namespace FTD2XX_NET
{
    ...
    public class FTDI
    {
        private const string dllname = "FTD2XX.DLL"; // NOTE : This will be case-sensitive under Linux
		...
		public FTDI()
		{
			...
			if (hFTD2XXDLL == IntPtr.Zero)
			{
				...
				hFTD2XXDLL = LoadLibrary(dllname);
				if (hFTD2XXDLL == IntPtr.Zero)
				{
                    ...
                    string dir = System.IO.Path.GetDirectoryName(GetType().Assembly.Location);
                    string filename = System.IO.Path.Combine(dir, dllname);

					hFTD2XXDLL = LoadLibrary(filename);
				}
			}

			// If we have successfully loaded the library, get the function pointers set up
			if (hFTD2XXDLL != IntPtr.Zero)
			{
        ...
        pFT_SetVIDPID = GetProcAddress(hFTD2XXDLL, "FT_SetVIDPID");
			...
			else
			{
                ...
                ReportError("Failed to load FTD2XX.DLL.  Are the FTDI drivers installed?");
			}
		}
    ...
    //+++
		...
		{
			/// <summary>
            /// Default Constructor for FT_EXCEPTION.
			/// </summary>
			public FT_EXCEPTION() { }

			/// <summary>
            /// Alternate constructor for FT_EXCEPTION taking a message string.
			/// </summary>
			/// <param name="message">The message string describing the exception.</param>
			public FT_EXCEPTION(string message) : base(message) { }
			/// <summary>
            /// Alternate constructor for FT_EXCEPTION taking a message string and inner exception.
			/// </summary>
            /// <param name="message">The message string describing the exception.</param>
			/// <param name="inner">The inner exception</param>
			public FT_EXCEPTION(string message, Exception inner) : base(message, inner) { }
			/// <summary>
            /// An alternate constructor forFT_EXCEPTION taking a SerializationInfo and StreamingContext. 
			/// </summary>
			/// <param name="info">A SerializationInfo.</param>
    ...
    IntPtr pFT_SetVIDPID = IntPtr.Zero; //+++
		...
		public FT_STATUS GetNumberOfDevices(ref UInt32 devcount)
		{
			...
			{
				ReportError("Failed to load function FT_CreateDeviceInfoList.");
			}
			return ftStatus;
		...
		public FT_STATUS GetDeviceList(FT_DEVICE_INFO_NODE[] devicelist)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_GetDeviceInfoListDetail.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS OpenByIndex(UInt32 index)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_Open.");
				}
				if (pFT_SetDataCharacteristics == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetDataCharacteristics.");
				}
				if (pFT_SetFlowControl == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetFlowControl.");
				}
				if (pFT_SetBaudRate == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetBaudRate.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS OpenBySerialNumber(string serialnumber)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_OpenEx.");
				}
				if (pFT_SetDataCharacteristics == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetDataCharacteristics.");
		...
		public FT_STATUS Close()
		{
			...
			else
			{
				if (pFT_Close == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_Close.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Read(byte[] dataBuffer, UInt32 numBytesToRead, ref UInt32 numBytesRead)
		{
			...
			else
			{
				if (pFT_Read == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_Read.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Write(byte[] dataBuffer, Int32 numBytesToWrite, ref UInt32 numBytesWritten)
		{
			...
			else
			{
				if (pFT_Write == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_Write.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ResetDevice()
		{
			...
			else
			{
				if (pFT_ResetDevice == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_ResetDevice.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Purge(UInt32 purgemask)
		{
			...
			else
			{
				if (pFT_Purge == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_Purge.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetEventNotification(UInt32 eventmask, EventWaitHandle eventhandle)
		{
			...
			else
			{
				if (pFT_SetEventNotification == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetEventNotification.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS StopInTask()
		{
			...
			else
			{
				if (pFT_StopInTask == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_StopInTask.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS RestartInTask()
		{
			...
			else
			{
				if (pFT_RestartInTask == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_RestartInTask.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ResetPort()
		{
			...
			else
			{
				if (pFT_ResetPort == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_ResetPort.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS CyclePort()
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_CyclePort.");
				}
				if (pFT_Close == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_Close.");
		...
		public FT_STATUS Rescan()
		{
			...
			else
			{
				if (pFT_Rescan == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_Rescan.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS Reload(UInt16 VendorID, UInt16 ProductID)
		{
			...
			else
			{
				if (pFT_Reload == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_Reload.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetBitMode(byte Mask, byte BitMode)
		{
			...
			else
			{
				if (pFT_SetBitMode == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetBitMode.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetPinStates(ref byte BitMode)
		{
			...
			else
			{
				if (pFT_GetBitMode == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetBitMode.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ReadEEPROMLocation(UInt32 Address, ref UInt16 EEValue)
		{
			...
			else
			{
				if (pFT_ReadEE == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_ReadEE.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS WriteEEPROMLocation(UInt32 Address, UInt16 EEValue)
		{
			...
			else
			{
				if (pFT_WriteEE == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_WriteEE.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS EraseEEPROM()
		{
			...
			else
			{
				if (pFT_EraseEE == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_EraseEE.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS ReadFT232BEEPROM(FT232B_EEPROM_STRUCTURE ee232b)
		{
			...
			else
			{
				if (pFT_EE_Read == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_EE_Read.");
				}
			}
			return ftStatus;
		}
		...
		/// </summary>
        /// <param name="ee232r">An FT232R_EEPROM_STRUCTURE structure to contain the data to be read from EEPROM.</param>
		...
		public FT_STATUS WriteFT232BEEPROM(FT232B_EEPROM_STRUCTURE ee232b)
		{
			...
			else
			{
				if (pFT_EE_Program == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_EE_Program.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS EEReadUserArea(byte[] UserAreaDataBuffer, ref UInt32 numBytesRead)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_EE_UASize.");
				}
				if (pFT_EE_UARead == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_EE_UARead.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS EEWriteUserArea(byte[] UserAreaDataBuffer)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_EE_UAWrite.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetDeviceType(ref FT_DEVICE DeviceType)
		{
			...
			else
			{
				if (pFT_GetDeviceInfo == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetDeviceInfo.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetRxBytesAvailable(ref UInt32 RxQueue)
		{
			...
			else
			{
				if (pFT_GetQueueStatus == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetQueueStatus.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetTxBytesWaiting(ref UInt32 TxQueue)
		{
			...
			else
			{
				if (pFT_GetStatus == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetStatus.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetModemStatus(ref byte ModemStatus)
		{
			...
			else
			{
				if (pFT_GetModemStatus == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetModemStatus.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetRTS(bool Enable)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_SetRts.");
				}
				if (pFT_ClrRts == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_ClrRts.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetDTR(bool Enable)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_SetDtr.");
				}
				if (pFT_ClrDtr == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_ClrDtr.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetTimeouts(UInt32 ReadTimeout, UInt32 WriteTimeout)
		{
			...
			else
			{
				if (pFT_SetTimeouts == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetTimeouts.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetBreak(bool Enable)
		{
			...
			else
			{
				...
				{
					ReportError("Failed to load function FT_SetBreakOn.");
				}
				if (pFT_SetBreakOff == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetBreakOff.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetResetPipeRetryCount(UInt32 ResetPipeRetryCount)
		{
			...
			else
			{
				if (pFT_SetResetPipeRetryCount == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetResetPipeRetryCount.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetDriverVersion(ref UInt32 DriverVersion)
		{
			...
			else
			{
				if (pFT_GetDriverVersion == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetDriverVersion.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetLibraryVersion(ref UInt32 LibraryVersion)
		{
			...
			else
			{
				if (pFT_GetLibraryVersion == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetLibraryVersion.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetDeadmanTimeout(UInt32 DeadmanTimeout)
		{
			...
			else
			{
				if (pFT_SetDeadmanTimeout == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetDeadmanTimeout.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetLatency(byte Latency)
		{
			...
			else
			{
				if (pFT_SetLatencyTimer == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetLatencyTimer.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetLatency(ref byte Latency)
		{
			...
			else
			{
				if (pFT_GetLatencyTimer == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetLatencyTimer.");
				}
			}
			return ftStatus;
		}
		...
		//public UInt32 InTransferSize(UInt32 InTransferSize, UInt32 OutTransferSize)
		{
			...
			else
			{
				if (pFT_SetUSBParameters == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetUSBParameters.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS SetCharacters(byte EventChar, bool EventCharEnable, byte ErrorChar, bool ErrorCharEnable)
		{
			...
			else
			{
				if (pFT_SetChars == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_SetChars.");
				}
			}
			return ftStatus;
		}
		...
		public FT_STATUS GetCOMPort(out string ComPortName)
		{
			...
			else
			{
				if (pFT_GetComPortNumber == IntPtr.Zero)
				{
					ReportError("Failed to load function FT_GetComPortNumber.");
				}
			}
			return ftStatus;
		}
